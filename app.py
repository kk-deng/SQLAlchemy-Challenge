...
@app.route("/")
def main():    
    return 'Welcome to Hawaii API!<br>\
        ...
        <a href="/api/v1.0/20170101" target="_blank">/api/v1.0/20170101</a><br>\
        <a href="/api/v1.0/20170101/20170801" target="_blank">/api/v1.0/20170101/20170801</a><br>'
...
def start_route(start): 
    ...
    # Filter station?
    results_4 = session.query(Measurement.tobs).filter(Measurement.date >= start_date).all()
    dict_results_4 = {
        "TMin": np.min(results_4),
        "TAvg": np.mean(results_4),
        "TMax": np.max(results_4)
    }
    session.close()
    return jsonify(dict_results_4)
...
def start_end_route(start, end): 
    ...
    if start >= end:
        ...
        results_5 = session.query(Measurement.tobs).filter(Measurement.date >= start_date)\
            .filter(Measurement.date <= end_date).all()
        
        dict_results_5 = {
            "TMin": np.min(results_5),
            "TAvg": np.mean(results_5),
            "TMax": np.max(results_5)
        }
        session.close()
        return jsonify(dict_results_5)
